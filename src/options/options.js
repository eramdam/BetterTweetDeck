...
BHelper.settings.getAll(settings => {
      ...
      forEach(val, (value, keyname) => {
        ...
        if (key === "flash_tweets") {
          if (settings.flash_tweets) {
            ...
            $('input[name="flash_tweets.enabled"] ~ ul input').removeAttr(
              "disabled"
            );
          }
          ...
          $(
            `input[name="flash_tweets.mode"]#${settings.flash_tweets.mode}`
          ).prop("checked", settings.flash_tweets.enabled && true);
        }
        ...
        if (key === "ctrl_changes_interactions") {
          if (settings.ctrl_changes_interactions.enabled) {
            ...
            $(
              'input[name="ctrl_changes_interactions.enabled"] ~ select'
            ).removeAttr("disabled");
          }
        ...
        if (key === "custom_columns_width") {
          if (settings.custom_columns_width) {
            ...
            $(
              'input[name="custom_columns_width.enabled"] ~ ul input'
            ).removeAttr("disabled");
          }
  ...
  $("input[name], select[name]").on("change input", e => {
    ...
    if (e.target.type === "checkbox" && e.target.hasAttribute("data-ghost")) {
      const els = $(
        `[data-ghost][name="${e.target.name}"] ~ ul input, [data-ghost][name="${
          e.target.name
        }"] ~ select`
      );
...
$(
  ".sidebar-nav:first-child a:first-child, .content-block:first-child"
).addClass("-selected");
...
$('.sidebar-nav a[href^="#"]').on("click", ev => {
  ...
  $(`.sidebar-nav a[href="#${href}"], .content-block#${href}`).addClass(
    "-selected"
  );
});
...
const switchSettingPage = page => {
  $(".sidebar-nav a, .content-block").removeClass("-selected");
  $(`.sidebar-nav a[href="#${page}"], .content-block#${page}`).addClass(
    "-selected"
  );
};
...
// Get GitHub infos
fetch("https://api.github.com/repos/eramdam/BetterTweetDeck/contributors").then(
  res => {
    res.json().then(json => {
