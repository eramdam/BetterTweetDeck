...
[...document.querySelectorAll('[data-lang]')].forEach((el) => {
  ...
  el.innerHTML = BHelper.getMessage(msg) || el.innerHTML;
});
...
$('#import_settings_input').change((ev) => {
  ...
  reader.onload = (file) => {
    ...
    try {
      ...
      alert(
        "Your settings file could not be imported. Make sure it wasn't edited or alterted.");
    }

    BHelper.settings.set(json);
...
$('#export_settings_button').click(() => {
  BHelper.settings.getAll((exportedSettings) => {
    exportedSettings.$$IS_FROM_BTD = true;
    exportedSettings.$$BTD_NOTICE = `DO NOT EDIT THIS FILE MANUALLY, OTHERWISE YOU'RE RUNNING THE RISK OF WEIRD BEHAVIORS, BUGS, OR BREAKAGE OF BETTER TWEETDECK ITSELF.
    
    IF SOMEHOW YOU BREAK SOMETHING BECAUSE YOU EDITED THAT FILE UPON RE-IMPORTING YOUR SETTINGS, YOU'RE ON YOUR OWN! SORRY!`;
    ...
    FileSaver.saveAs(
      blob,
      `better-tweetdeck-settings-${BHelper.getVersion()}.json`,
    );
  });
});
